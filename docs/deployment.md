...
## Состояние на 26 июля 2026 года

На хосте уже выполнено:

- репозиторий клонирован в `/opt/squadjs_ostw/autoseed`;
- Node.js `24.18.0` и Yarn `1.22.22` установлены в `.runtime/node` с проверкой
  SHA-256;
- совместимый снимок SquadJS
  `ac96f3afcc2595a1b2463887ee1658e730dfdbe1` размещён в `squadjs`;
- безопасный `overlay` установлен, зависимости SquadJS собраны;
- импорт `AutoseedExporter` и все 13 проверок комплекта проходят на хосте;
- создан закрытый незаполненный `deploy/.env` с правами `0600`.

Процессы PM2 намеренно не запущены: без RCON, журналов и ключа они не смогут
перейти в состояние готовности. Порты экспортёров `32081` и `32082` оставлены
свободными.

Установщик SquadJS сообщает об устаревшей транзитивной зависимости
`basic-ftp@4.6.6`. Подготовленные конфигурации используют локальный режим
`tail`, поэтому FTP не включается. Переходить на FTP следует только после
обновления зависимости в исходном SquadJS и повторной проверки совместимости.
...
5. включение автозапуска PM2 для пользователя `ostw` после приёмки процессов.
